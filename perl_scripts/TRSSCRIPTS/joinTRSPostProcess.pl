	...
	if(@l eq 1){
	   ...
	   my $str = $STRINGS->{$end};
		...
		if($scaff2 ne $scaff1){
			...
			my $GENOMEA = $infoann->{$A} or die ;
			my $GENOMEB = $infoann->{$B} or die ;
		...
		if(1){
		   ...
		   my ($fe,$x) =  util_readfasta("FASTADIR_ORFNEW/$end.ALL.1.fasta");
		   my ($fb,$y) =  util_readfasta("FASTADIR_ORFNEW/$begin.ALL.1.fasta");
